...
class ContentApiTests(rest_test.APITestCase):
    ...
    def test_can_return_traditional_chinese_alert(self):
        alert = AlertBuilder().build()
    ...
    def test_can_return_simplified_chinese_alert(self):
        alert = AlertBuilder().build()
        ...
        url = '/v1/content/alerts/zh_CN/{0}/'.format(alert_from_db.pk)
        response = self.client.get(url)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.json()['heading'], 'Simplified Chinese Title')
        self.assertEqual(response.json()['content'], 'Simplified Chinese description')
