...
class TestImportBc211Data(TestCase):
    def test_import_one_record(self):
        ...
        self.assertIn('Successfully imported 1 organization(s), 1 location(s), and 27 taxonomy term(s)',
                      out.getvalue())

    def test_import_many_records(self):
        out = StringIO()
        call_command('import_bc211_data', MULTI_AGENCY_FIXTURE, stdout=out)
        self.assertIn('Successfully imported 16 organization(s), 40 location(s), and 552 taxonomy term(s)', out.getvalue())
