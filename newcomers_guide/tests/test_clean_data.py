...
class CleanUpUrlLinksTest(TestCase):
    def test_replaces_http_link_with_markdown(self):
        ...
        self.assertEqual(clean_up_http_links(text), 'abc [http://example.com](http://example.com) def')

    def test_replaces_two_links_with_markdown(self):
        text = 'abc http://example.com def http://example.org ghi'
        expected = 'abc [http://example.com](http://example.com) def [http://example.org](http://example.org) ghi'
        self.assertEqual(clean_up_http_links(text), expected)

    def test_replaces_https_link_with_markdown(self):
        text = 'https://example.com'
        self.assertEqual(clean_up_http_links(text), '[https://example.com](https://example.com)')

    def test_handles_urls_with_dash_in_the_host_name(self):
        text = 'http://www.cra-arc.gc.ca/'
        expected = '[http://www.cra-arc.gc.ca/](http://www.cra-arc.gc.ca/)'
        self.assertEqual(clean_up_http_links(text), expected)
    ...
    def test_http_link_does_not_truncate_equals_sign(self):
        ...
        self.assertEqual(clean_up_http_links(text), 'abc [http://example.com=434](http://example.com=434) def')
    ...
    def test_http_link_does_not_truncate_query(self):
        ...
        self.assertEqual(clean_up_http_links(text), 'abc [http://foo.com/find?a=a](http://foo.com/find?a=a) def')
    ...
    def test_http_link_does_not_truncate_query_with_ampersand(self):
        ...
        self.assertEqual(clean_up_http_links(
            text), 'abc [http://foo.com/find?a=a&b=b](http://foo.com/find?a=a&b=b) def')

    def test_http_link_does_not_truncate_path_ending_with_word_character(self):
        text = 'abc http://example.com/search def'
        self.assertEqual(clean_up_http_links(
            text), 'abc [http://example.com/search](http://example.com/search) def')
    ...
    def test_http_link_with_parentheses(self):
        ...
        expected = 'abc [https://www.welcomebc.ca//Re...](https://www.welcomebc.ca//Resources-For/For-regulators/Foreign-Qualifications-Recognition-(FQR)) def'
        self.assertEqual(clean_up_http_links(text), expected)

    def test_http_link_with_dash(self):
        text = 'abc http://www.leg.bc.ca/mla/3-1-1.htm def'
        expected = 'abc [http://www.leg.bc.ca/mla/3-1...](http://www.leg.bc.ca/mla/3-1-1.htm) def'
        self.assertEqual(clean_up_http_links(text), expected)

    def test_http_link_with_underscore(self):
        text = 'abc https://www.bced.gov.bc.ca/home_school def'
        expected = 'abc [https://www.bced.gov.bc.ca/h...](https://www.bced.gov.bc.ca/home_school) def'
        self.assertEqual(clean_up_http_links(text), expected)

    def test_http_link_with_question_mark(self):
        text = 'abc https://www.cba.ca/?cat=Banking-Basics def'
        expected = 'abc [https://www.cba.ca/?cat=Bank...](https://www.cba.ca/?cat=Banking-Basics) def'
        self.assertEqual(clean_up_http_links(text), expected)

    def test_http_link_with_trailing_slash(self):
        text = 'abc https://www.bchrt.bc.ca/ def'
        expected = 'abc [https://www.bchrt.bc.ca/](https://www.bchrt.bc.ca/) def'
        self.assertEqual(clean_up_http_links(text), expected)

    def test_http_link_surrounded_by_chinese_characters(self):
        text = '他們可協https://www.cic.gc.ca/他們可協'
        expected = '他們可協[https://www.cic.gc.ca/](https://www.cic.gc.ca/)他們可協'
        self.assertEqual(clean_up_http_links(text), expected)

    def test_http_link_surrounded_by_punjabi_characters(self):
        text = 'https://www.carproof.comਤੋਂ'
        expected = '[https://www.carproof.com](https://www.carproof.com)ਤੋਂ'
        self.assertEqual(clean_up_http_links(text), expected)
    ...
    def test_chinese_1(self):
        ...
        expected = '会有公[http://www.civicinfo.bc.ca/m...](http://www.civicinfo.bc.ca/municipalities)地方区域或会有公园及康乐资料。[http://www.civicinfo.bc.ca/r...](http://www.civicinfo.bc.ca/regionaldistricts)会有公'
        self.assertEqual(clean_up_http_links(text), expected)
