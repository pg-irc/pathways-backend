...
class ImportMissingCoordinatesTests(TestCase):
    ...
    def test_can_replace_missing_coordinates_with_latlong_from_city(self):
        location_id = a_string()
        location_without_point = LocationBuilder(self.organization).with_id(location_id).with_point(None).create()
        address = AddressBuilder().with_city('Vancouver').create()
        LocationAddress(address=address, location=location_without_point, address_type=self.physical_address_type).save()
    ...
    def test_does_not_replace_coordinates_when_they_are_not_missing(self):
        location_id = a_string()
        location_with_point = LocationBuilder(self.organization).with_id(location_id).with_point(Point(-123.500, 49.500)).create()
        address = AddressBuilder().with_city('Vancouver').create()
        LocationAddress(address=address, location=location_with_point, address_type=self.physical_address_type).save()
