...
-- For use in situations when we have a list of interesting service ids in a file and we want to pull data
-- just on those. Create a table and copy data from the file:

-- # create table interesting_ids (id int);
-- # copy interesting_ids from '/path/to/interesting_ids.txt';

-- then add this to the WHERE clause below:

-- service.id IN (select cast(id as text) from interesting_ids)
