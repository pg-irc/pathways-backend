...
copy (
	select topic.master_id as topic_id,
		max(topic.name) as topic_title,
		string_agg(concat(taxonomy_terms.taxonomy_id, ':', taxonomy_terms.name),', ') as taxonomy_term,
		max(topic.description) as topic_content
